...
def define_E(nlatent, input_nc, nef, norm='batch', gpu_ids=[], image_size=64):
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
...
class LatentEncoder(nn.Module):
    def __init__(self, nlatent, input_nc, nef, norm_layer, gpu_ids=[], image_size=64):
        ...
        if image_size == 64:
            ...
            sequence.extend(
                [
                    nn.Conv2d(8 * nef, 16 * nef, kernel_size=4, stride=1, padding=0, bias=use_bias),
                    norm_layer(16 * nef),
                    nn.ReLU(True),

                    nn.Conv2d(16 * nef, 16 * nef, kernel_size=5, stride=1, padding=0, bias=use_bias),
                    norm_layer(16 * nef),
                    nn.ReLU(True)
                ]
            )
            ...
            self.conv_modules = nn.Sequential(*sequence)
            self.enc_mu = nn.Conv2d(16 * nef, nlatent, kernel_size=1, stride=1, padding=0, bias=True)
            self.enc_logvar = nn.Conv2d(16 * nef, nlatent, kernel_size=1, stride=1, padding=0, bias=True)
        else:
