...
from edges2shoes_data import load_edges2shoes, AlignedIterator, UnalignedIterator
...
def visualize_cycle_B_multi(opt, real_B, model, name='cycle_B_multi_test.png'):
    size = real_B.size()
    multi_prior_z_B = Variable(real_B.data.new(opt.num_multi, opt.nlatent, 1, 1).normal_(0, 1).repeat(size[0], 1, 1, 1),
                               volatile=True)
...
def visualize_multi(opt, real_A, model, name='multi_test.png'):
    ...
    multi_prior_z_B = Variable(real_A.data.new(opt.num_multi, opt.nlatent, 1, 1).normal_(0, 1).repeat(size[0], 1, 1, 1),
                               volatile=True)
...
if __name__ == "__main__":
    # CUDA_VISIBLE_DEVICES=0 python -m ipdb test.py --chk_path checkpoints/FOLDER/latest
    # --res_dir val_res --opt_path checkpoints/FOLDER/opt.txt
    test_model()
