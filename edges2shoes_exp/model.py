...
def criterion_GAN(pred, target_is_real, use_sigmoid=True):
    if use_sigmoid:
        if target_is_real:
            target_var = Variable(pred.data.new(pred.size()).float().fill_(1.))
        else:
            target_var = Variable(pred.data.new(pred.size()).float().fill_(0.))
...
class StochCycleGAN(object):
    ...
    def train_instance(self, real_A, real_B, prior_z_B):
        ...
        # generate B using A and z_B : A ==> B <== z_B
...
class AugmentedCycleGAN(object):
    ...
    def train_instance(self, real_A, real_B, prior_z_B):
        ...
        # Return dicts
        losses = OrderedDict([('D_A', loss_D_A.item()), ('G_A', loss_G_A.item()),
                              ('Cyc_A', loss_cycle_A.item()),
                              ('Cyc_z_B', loss_cycle_z_B.item()), ('KLD_z_B', kld_z_B.item()),
                              ('D_B', loss_D_B.item()), ('G_B', loss_G_B.item()),
